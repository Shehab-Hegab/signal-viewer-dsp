...
# "sys" module from the Python standard library to interact with the system and perform various tasks (eg. Open & Close)
import sys
...
import pandas as pd
# QTimer is a class provided by the PyQt5 library (PyQt for Python) that allows you to create timer objects
...
class SignalViewer(QMainWindow):
    ...
    def open_dialog_box(self, graphic_channel,button, timer, repeat_checkbox):
        ...
        if path:
            signal_widget = Signal(path)
            container_widget = QWidget()
            signal_widget = Signal(path, color)
            self.signals.append(signal_widget)  


        if path:
            if button == self.BrowseButton:
                self.signal_widget1 = Signal(path)
                container_widget = QWidget()
                ...
                mean, variance, peak, trough = self.signal_widget1.calculate_statistics()
                self.StatisticsField.setText(f"Mean: {mean:.2f}, Variance: {variance:.2f}\nPeak: {peak:.2f}, Trough: {trough:.2f}")
            ...
            if button == self.BrowseButton2:
                self.signal_widget2 = Signal(path)
                container_widget = QWidget()
                ...
                graphic_channel.setScene(QGraphicsScene())
                graphic_channel.layout().addWidget(container_widget)
                mean, variance, peak, trough = self.signal_widget2.calculate_statistics()
                self.StatisticsField2.setText(f"Mean: {mean:.2f}, Variance: {variance:.2f}\nPeak: {peak:.2f}, Trough: {trough:.2f}")
                ...
                if path.endswith('.dat'):
                    ...
                    if record.sig_name:
                        ...
                        def on_channel_change(index):
                            selected_channel_index = int(index)
                            if selected_channel_index >= 0:
                                if graphic_channel == self.Channel1Graphic and self.signal_widget2:
                                    self.signal_widget2.load_signal(path, selected_channel_index)
                ...
                self.ChannelsComboBox.setCurrentIndex(0)
                mean, variance, peak, trough = self.signal_widget.calculate_statistics()
                self.StatisticsField.setText(f"Mean: {mean:.2f}, Variance: {variance:.2f}\nPeak: {peak:.2f}, Trough: {trough:.2f}")
                repeat_checkbox.stateChanged.connect(lambda state: self.set_repeat_enabled(self.signal_widget, state))
    ...
    # [6] Clode Button Handling Function
    # ----------------------------------
    ...
    def move_signal_down_handler(self):
        ...
        if layout1 and layout1.count() > 0:
            ...
            if not layout2:
                layout2 = QVBoxLayout()
                self.Channel2Graphic.setLayout(layout2)
            else:
                while layout2.count() > 0:
                    layout2_item = layout2.takeAt(0)
                    if layout2_item.widget():
                        layout2_item.widget().deleteLater()
            layout2.addWidget(widget)
            self.Channel1Graphic.setSceneRect(0, 0, 0, 0)
            self.Channel2Graphic.setSceneRect(0, 0, widget.width(), widget.height())
    ...
    def move_signal_up_handler(self):
        ...
        if layout2 and layout2.count() > 0:
            ...
            if not layout1:
                layout1 = QVBoxLayout()
                self.Channel1Graphic.setLayout(layout1)
            else:
                while layout1.count() > 0:
                    layout1_item = layout1.takeAt(0)
                    if layout1_item.widget():
                        layout1_item.widget().deleteLater()
            layout1.addWidget(widget)
            self.Channel2Graphic.setSceneRect(0, 0, 0, 0)
            self.Channel1Graphic.setSceneRect(0, 0, widget.width(), widget.height())
    ...
    # [9] Update Some Buttons WHEN Link Button Cliked
    # -----------------------------------------------
...
class Signal:
    def __init__(self, path,color=None):
        ...
        self.curve = self.plot_item.plot(pen=self.color if color else 'w')
    ...
    # [14] Load Any Type of Signals
    # -----------------------------
    def load_signal(self, path, selected_channel_index):
        if path.endswith('.dat'):
            record = wfdb.rdrecord(path[:-4], channels=[selected_channel_index])
    ...
    # [15] Statistics Calculation Function For Each Uploaded Signal
    # -------------------------------------------------------------
    def calculate_statistics(self):
        mean = np.mean(self.ecg_signal)
        variance = np.var(self.ecg_signal)
        peak = np.max(self.ecg_signal)
        trough = np.min(self.ecg_signal)
        return mean, variance, peak, trough



    # [16] Update The Graphic View
    # ----------------------------
    def update(self, timer):
        if not self.deleted and self.playing and self.ptr < len(self.time):
            x_max = self.time[self.ptr]
            x_min = max(0, x_max - self.fixed_x_range)

            if not self.curve_deleted and self.curve:
                self.curve.setData(self.time[:self.ptr+1], self.ecg_signal[:self.ptr+1])
    ...
    # [17] Delete (Remove) The Displayed Signal
    # -----------------------------------------
    def delete(self):
        self.deleted = True
        self.playing = False
        self.curve_deleted = True
